...
function HorizontalNav({ label, href, subCategories }: NavProp) {
  ...
  return (
    <div className="relative">
      {!hasSubCategories ? (
        <Link
          className="peer flex items-center py-4 text-md font-semibold text-gray-500 hover:text-cyan-500 dark:text-gray-400"
      ...
      {hasSubCategories && (
        <div className="absolute right-0 z-10 hidden w-[200px] flex-col border-t-2 border-cyan-500 bg-[#f6f1eb] text-gray-500 shadow-md drop-shadow-lg hover:flex peer-hover:flex dark:bg-[#2d333b] dark:text-gray-400">
          {subCategories.map((subCategory) => (
            <Link
              className={`px-5 py-3 text-md font-semibold hover:bg-[#eae4dd] dark:hover:bg-gray-500 border-b-2 ${router.asPath === subCategory.href ? 'border-cyan-500' : 'border-transparent'}`}
