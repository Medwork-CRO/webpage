...
function Showcasing(
    {
        title, bullets = false, textSize, smallTextSize, showcases
    }: {
        title: string, bullets?: boolean, textSize?: string, smallTextSize?: string, showcases: Showcase[]
    }
) {
    const variants = {
            ...
            `}>
                {showcases.map((showcase, i) => (
                    <li className="narrow-letters leading-relaxed" key={i}>
                        <p className={`font-normal ${textSize ?? "text-2xl md:text-3xl"}`}>{showcase.title}</p>
                        <p className={`font-thin ${smallTextSize ?? "text-xl"}`}>{showcase.point}</p>
                    </li>
                ))}
            </ul>
        </motion.div>
