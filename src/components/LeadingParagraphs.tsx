...
function LeadingParagraphs({
  paragraphs,
  withLead = false,
}: LeadingParagraphsProps) {
  const variants = {
    ...
    >
      {paragraphs.map((paragraph, index) =>
        !index ? (
          <p
            key={index}
            className={`${index === paragraphs.length - 1 ? "" : "mb-6 sm:mb-8"} ${withLead ? "font-light text-2xl md:text-3xl" : "font-thin text-xl md:text-2xl"}`}
          >
            {paragraph}
          </p>
