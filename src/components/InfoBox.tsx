...
const InfoBox = ({ title, text, link, color }: InfoBoxProps) => (
    <Link href={link}>
        ...
        >
            <h1 className="mb-2 text-3xl sm:text-2xl font-ultralight narrow-letters leading-relaxed ">{title.toUpperCase()}</h1>
            <p className="text-xl sm:text-lg font-ultralight narrow-letters leading-relaxed ">{text}</p>
        </div>
    </Link>
);
